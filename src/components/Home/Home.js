...
import { FormControl, MenuItem, Select, InputLabel, TextField, Container, CircularProgress } from '@material-ui/core';
...
const Home = () => {
	...
	const [districtCode, setDistrictCode] = useState('PLEASE SELECT A STATE FIRST');
	...
	const currentVaccine = vaccineData.slice(indexOfFirstVaccine, indexOfLastVaccine);
	...
	const onStateChange = async (e) => {
		...
		const url =
			stateCode === 'States' ? null : `https://cdn-api.co-vin.in/api/v2/admin/location/districts/${stateCode}`;
		setLoading(true);
		await fetch(url)
			.then((res) => res.json())
			.then((data) => {
			...
			await fetch(
				...
				.then((data) => {
					const pincodeData = data?.centers?.map((res) => ({
						...
						from: res?.from,
						to: res?.to,
						available_capacity: res?.sessions?.slice(0, 1).map((res) => res?.available_capacity),
						date: res?.sessions?.slice(0, 1).map((res) => res?.date),
						min_age_limit: res?.sessions?.slice(0, 1).map((res) => res?.min_age_limit),
			...
			<Container maxWidth="md">
				<div className="home">
					...
					<div className="home_selectionHeader">
						...
						<FormControl>
							<InputLabel id="select-outlined-label">Search Criteria</InputLabel>
							<Select
								variant="filled"
								value={toSearchValue}
								onChange={(e) => {
							...
							>
								{toSearch.map((functionName, index) => {
									return (
										<MenuItem className="search__values" key={index} value={functionName}>
											{functionName}
										</MenuItem>
									);
								})}
					...
					{toSearchValue === '' && <h3 className="empty_error">Please Select an Option</h3>}

					{toSearchValue === 'Find By District' ? (
						<div className="home_selectedHeaders">
							<FormControl className="form-control">
								<Select variant="outlined" value={stateCode} onChange={onStateChange}>
									<MenuItem value="States">Select a State</MenuItem>
									{state?.map((stateData) => (
										<MenuItem value={stateData?.state_id}>{stateData?.state_name}</MenuItem>
									))}
								</Select>
							</FormControl>
							<FormControl className="form-control">
								{districts?.length !== 0 ? (
									<>
										<Select variant="outlined" value={districtCode} onChange={findByDistrict}>
